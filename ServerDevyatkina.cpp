...
// ИСПРАВЛЕНО: тип изменён на recursive_mutex
std::recursive_mutex ServerDevyatkina::m_mx;
std::map<int, std::shared_ptr<SessionDevyatkina>> ServerDevyatkina::m_sessions;
std::map<int, std::shared_ptr<boost::asio::ip::tcp::socket>> ServerDevyatkina::m_clients;
std::map<int, std::thread> ServerDevyatkina::m_workers;
std::shared_ptr<boost::asio::ip::tcp::acceptor> ServerDevyatkina::m_acceptor;
...
void ServerDevyatkina::clientHandler(int clientId,
...
void ServerDevyatkina::handleClientMessage(MessageDevyatkina& msg, int clientId)
{
    ...
    case MT_CONNECT_DEVYATKINA:
    {
        ...
        if (socket)
        {
            MessageDevyatkina confirm(MT_CONFIRM_DEVYATKINA,
                std::to_wstring(clientId), clientId, -2);
    ...
    case MT_DATA_DEVYATKINA:
    {
        if (msg.header.to == -1)
        {
            ...
            for (int id : ids)
            {
                ...
                out.header.to = id;
                // ИСПРАВЛЕНО: передаём id получателя
        ...
        else
        {
            ...
            if (exists)
            {
                MessageDevyatkina out = msg;
                out.header.from = clientId;
                // ИСПРАВЛЕНО: передаём out.header.to
...
void ServerDevyatkina::worker(int clientId)
{
    while (true)
    {
        ...
        // ИСПРАВЛЕНО: receive возвращает false при stop() -> выходим из цикла
        if (!LocalTransportDevyatkina(clientId).receive(msg))
            break;
...
void ServerDevyatkina::broadcastSessions()
{
    std::vector<int> ids;
    {
        // ИСПРАВЛЕНО: мьютекс освобождается ДО цикла рассылки
        std::lock_guard lg(m_mx);
...
void ServerDevyatkina::removeClient(int clientId, bool notifyClient)
{
...

    {
        ...
        // ИСПРАВЛЕНО: сразу удаляем из m_sessions и LocalTransport
        m_sessions.erase(clientId);
        LocalTransportDevyatkina::eraseSession(clientId);
    }
    ...
    if (session)
    {
        MessageDevyatkina stopMsg(MT_DISCONNECT_DEVYATKINA, L"", clientId, -2);
        session->addMessage(stopMsg);
        session->stop();  // ИСПРАВЛЕНО: будим воркер чтобы вышел из wait()
    }
